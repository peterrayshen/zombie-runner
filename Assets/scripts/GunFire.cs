...
public class GunFire : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetButtonDown ("Fire1") && GlobalAmmo.loaded_ammo_pistol > 0 && rateReset) {
			...
			Debug.DrawRay (transform.position, transform.TransformDirection(Vector3.forward), Color.green, 10);
			AudioSource gunSound = GetComponent<AudioSource> ();
			gunSound.Play ();
			muzzleOn ();
			GetComponent<Animation> ().Stop("gunshot");
			GetComponent<Animation> ().Play ("gunshot");
			GlobalAmmo.loaded_ammo_pistol -= 1;
			timePassed = 0;
		} 
	}
